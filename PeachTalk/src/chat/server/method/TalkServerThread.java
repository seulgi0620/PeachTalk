						...
						if(obj instanceof String) {
							...
							if(msg != null) {
								...
								if(Protocol.enter_room.equals(protocol)) {
									...
									Vector<FriendsPageVO> v_fpvo = ft.search(user_id);
									//v_friendsName = new Vector<String>();
									//v_friendsProfile = new Vector<String>();
									for (int p = 0; p < v_fpvo.size(); p++) {
										//v_friendsName.add(v_fpvo.get(p).getFriendName());
										//v_friendsProfile.add(v_fpvo.get(p).getUser_profile());
										v_fpvo.get(p).setStatus("myfriend");
									}
									send(Protocol.msg(v_fpvo));
									
								}
								else if (Protocol.search_friend.equals(protocol)) {
									SearchTable stb = new SearchTable();
									Vector<FriendsPageVO> v_fpvo = stb.search(user_id,message);
									//v_searchID = new Vector<String>();
									for (int p = 0; p < v_fpvo.size(); p++) {
										//v_searchID.add(v_fpvo.get(p).getSearch_user_id());
										v_fpvo.get(p).setStatus("search");
									}
									send(Protocol.msg(v_fpvo));
								}
								else if (Protocol.addFriend.equals(protocol)) {
									SearchAddFriend saf = new SearchAddFriend();
									Vector<FriendsPageVO> v_fpvo = saf.friendAdd(user_id,message);
									//v_addID = new Vector<String>();
									for (int p = 0; p < v_fpvo.size(); p++) {
										//v_addID.add(v_fpvo.get(p).getSelected_user_id());
									}
									FriendsTable ft = new FriendsTable();
									v_fpvo = ft.search(user_id);
									//v_friendsName = new Vector<String>();
									for (int p = 0; p < v_fpvo.size(); p++) {
										//v_friendsName.add(v_fpvo.get(p).getFriendName());
										v_fpvo.get(p).setStatus("addfriend");
									}
									send(Protocol.msg(v_fpvo));
								}
